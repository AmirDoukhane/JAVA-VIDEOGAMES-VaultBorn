...
//        game.setBackgroundMusic("sounds/land_of_snow_background_music.mp3");
    ...
    public void createMenu(MainGame game, Skin skin, List<String> element) {
        ...
        float maxHeight = (Gdx.graphics.getHeight() * 0.85f - element.size() * 20f) / element.size();
        float buttonHeight = Math.min(Gdx.graphics.getHeight() * 0.2f, maxHeight);

        for (String e : element) {
            TextButton button = new TextButton(e, skin);
            button.getLabel().setFontScale(2f);
            table.add(button).pad(10).width(500).height(buttonHeight).row();
    ...
    public void onClick(String name) throws FactoryException {
        switch (name) {
            ...
            case "Poursuivre":
                if (SaveManager.hasSave()) {
                    game.setScreen(new LoadingScreen(game, skin));
                } else {
