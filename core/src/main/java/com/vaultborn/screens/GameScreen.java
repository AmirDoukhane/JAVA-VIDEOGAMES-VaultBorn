...
public class GameScreen implements Screen {
    ...
    private static final List<String> buttonPause = Arrays.asList("Continuer", "Sauvegarder", "Menu principal", "Parametres", "Exit");

    private float saveNotifTimer = 0f;
...
//            System.out.println("Partie sauvegardée !" + game);
        ...
        if (!PauseMenuScreen.isActivated()) {
            ...
            if (saveNotifTimer > 0) {
                saveNotifTimer -= delta;
                float alpha = Math.min(1f, saveNotifTimer);
                BitmapFont font = MainGame.getFont();
                font.setColor(0f, 1f, 0.4f, alpha);
                font.getData().setScale(1.5f);
                font.draw(batch, "Partie sauvegardee !", 20f, Gdx.graphics.getHeight() - 100f);
                font.getData().setScale(1f);
                font.setColor(Color.WHITE);
            }
            batch.end();

            if (game.recentlySaved) {
                saveNotifTimer = 2f;
                game.recentlySaved = false;
            }
